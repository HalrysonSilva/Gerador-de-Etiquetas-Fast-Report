...
begin
datamodule1.ETIQPRUNICO105X30mm.ShowReport;
end;
...
procedure Tsvetiquetas.EditquantidadeChange(Sender: TObject);
begin
ValidarQuantidade;
end;
...
procedure Tsvetiquetas.EditquantidadeKeyPress(Sender: TObject; var Key: Char);
begin

  // Permite apenas dígitos (0-9), Backspace (#8) e Delete (#127)
  if not (Key in ['0'..'9', #8, #127]) then
  begin
    Key := #0; // Anula a tecla se não for um número ou Backspace/Delete
  end;

end;

procedure Tsvetiquetas.ValidarQuantidade;
var
  qtd: Integer;
begin
  if TryStrToInt(EditQuantidade.Text, qtd) then
  begin
    if qtd > 126 then
    begin
      ShowMessage('A quantidade máxima permitida é 126. O valor será ajustado.');
      EditQuantidade.Text := '126';
      EditQuantidade.SelectAll; // Seleciona o texto para facilitar a correção
    end
    else if qtd < 1 then // Também é bom evitar quantidades menores que 1
    begin
      ShowMessage('A quantidade mínima permitida é 1. O valor será ajustado.');
      EditQuantidade.Text := '1';
      EditQuantidade.SelectAll;
    end;
  end
  else if (EditQuantidade.Text <> '') then // Se não for número e não estiver vazio
  begin
    ShowMessage('Por favor, digite apenas números para a quantidade.');
    EditQuantidade.Clear; // Limpa o campo se for texto inválido
    EditQuantidade.SetFocus;
  end;
end;
